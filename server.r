...
shinyServer(function(input, output) {
  ...
  an <- reactive({
    ...
    # For Hindi languge selection
    
    if('Hindi' %in% input$rb)
    {
      
      model <- udpipe_download_model(language = "Hindi")
      udmodel_hindi <- udpipe_load_model(file = 'hindi-ud-2.0-170801.udpipe')
      txt <- as.character(Dataset())
      txt <- udpipe_annotate(udmodel_hindi, x=txt)
      txt <- as.data.frame(txt)
      return(txt)
    } 
    
    # For spanish languge selection
  
    if('Spanish'  %in% input$rb)
    {
